...
import com.jmlearning.randomthings.raytracer.objects.*;
...
public class Scene {
    ...
    public boolean isShadowed(Vector toLight, Vector hitPoint) {
        ...
        double distSquared = toLight.lenSquared();
        
        for(Geometry primitive : primitives) {
            
            if(primitive.intersect(inRay, outRay)) {
                
                if(outRay.origin.sub(inRay.origin).lenSquared() < distSquared)
                    return true;
            }
        }
    ...
    private Geometry createBox(double xmin, double ymin, double zmin, double xmax, double ymax, double zmax, int color) {
    
        Geometry geometry = new Box(xmin, ymin, zmin, xmax, ymax, zmax);
        geometry.setMaterial(new Material(getAmbient(color), new Color(color), Color.WHITE));
    ...
    public Geometry createDisc(double x, double y, double z, double nx, double ny, double nz, double radius, int color) {
