...
public class LineRectIntersectionExample extends SimpleFramework {
    ...
    public LineRectIntersectionExample() {
        ...
        appHeight = 640;
        appSleep = 10L;
        appTitle = "Line Rectangle Intersection";
    ...
    protected void updateObjects(float delta) {
        ...
        for(int i = 0; i < rect.length; ++i) {
            ...
            rectCpy[i] = mat.multiply(rect[i]);
        }

        Vector2f d = end.subtract(start);
        float len = d.len();
        d = d.norm();
        Float t = lineRectIntersect(start, d, rectCpy);

        if(t != null && t > 0.0f && t < len)
            intersection = start.add(d.multiply(t));

        else
            intersection = null;
    }

    private Float lineRectIntersect(Vector2f O, Vector2f d, Vector2f[] rect) {
        ...
        for(int i = 0; i < 2; ++i) {
            ...
            Vector2f n = rect[i].subtract(rect[i + 1]);
            n = n.norm();
            float e0 = n.dot(rect[i].subtract(O));
            float e1 = n.dot(rect[i + 1].subtract(O));
            float f = n.dot(d);
    ...
    protected void render(Graphics g) {
        ...
        for(int i = 0; i < rectCpy.length; ++i) {
            ...
            rectCpy[i] = view.multiply(rectCpy[i]);
        }

        Utility.drawPolygon(g, rectCpy);
        Vector2f startCpy = view.multiply(start);
        Vector2f endCpy = view.multiply(end);
        g.drawLine((int) startCpy.x, (int) startCpy.y,
                (int) endCpy.x, (int) endCpy.y);

        if(hasIntersection) {

            g.setColor(Color.BLUE);
            Vector2f temp = view.multiply(intersection);
