...
public class PrototypeShip {
    ...
    public PrototypeShip(PolygonWrapper wrapper) {
        ...
        polyman = new Vector2f[] {new Vector2f(0.0325f, 0.0f),
    ...
    public PrototypeBullet launchBullet() {
        ...
        Vector2f bulletPosition = position.add(Vector2f.polar(angle, 0.0325f));

        return new PrototypeBullet(bulletPosition, angle);
    }
    ...
    private Vector2f[] transformPolygon() {
        ...
        Matrix3x3f matrix = Matrix3x3f.rotate(angle);
        matrix = matrix.multiply(Matrix3x3f.translate(position));

        return transform(polyman, matrix);
    }
