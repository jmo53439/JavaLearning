...
public class BouncingBallsExample extends SimpleFramework {
    ...
    private void createBalls() {
        ...
        for(int i = 0; i < balls.length; ++i) {
            ...
            balls[i] = new Ball();
            balls[i].velocity = new Vector2f(
                    random.nextFloat(), random.nextFloat());
            float r = 0.025f + random.nextFloat() / 8.0f;
            balls[i].position = new Vector2f(
                    WORLD_LEFT - r, WORLD_TOP - r);
            balls[i].radius = r;
    ...
    protected void render(Graphics g) {
        ...
        g.drawString("Press Up to increase ball count", 20, 65);
        g.drawString("Press Down to decrease ball count", 20, 80);
    }
