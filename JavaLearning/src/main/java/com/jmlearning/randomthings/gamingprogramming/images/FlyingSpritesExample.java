...
public class FlyingSpritesExample extends SimpleFramework {
    ...
    private void addTransparentBorder() {
        ...
        BufferedImage newSprite = new BufferedImage(
                borderWidth, borderHeight, BufferedImage.TYPE_INT_ARGB);
    ...
    protected void updateObjects(float delta) {
        ...
        for(int i = 0; i < positions.length; ++i) {
            ...
            positions[i] = positions[i].add(velocities[i].multiply(delta));
    ...
    protected void render(Graphics g) {
        ...
        super.render(g);
        g.drawString("(A)nti-Aliased: " + antialiased, 20, 35);
    ...
    private AffineTransform createTransform(Vector2f position, float angle) {
        ...
        Vector2f screen = getViewportTransform().multiply(position);
