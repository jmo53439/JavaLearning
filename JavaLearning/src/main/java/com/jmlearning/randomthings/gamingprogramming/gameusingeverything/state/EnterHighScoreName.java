...
public class EnterHighScoreName extends AttractState {
    ...
    public void enter() {
        ...
        Vector<GameLetter> temp = new Vector <>();
    ...
    private int addLetters(FontMetrics fm, int start, int end,
                           int maxWidth, Vector<GameLetter> letters) {
    ...
    private void updateHighScoreRecord(String letter) {
        ...
        if(letter.equalsIgnoreCase("end")) {
            ...
            highScoreManager.addNewScore(new Score(name, state.getScore()));
            finished = true;
        }
    ...
    public void render(Graphics2D g, Matrix3x3f view) {
        ...
        // draw score
        String score = "Score: " + highScoreManager.format(state.getScore());
        int scoreX = ((game.getScreenWidth() - fm.stringWidth(score)) / 2);
        int scoreY = sh;
        g.drawString(score, scoreX, scoreY);

        // draw the name
        int nameX = ((game.getScreenWidth() - fm.stringWidth(name)) / 2);
        int nameY = scoreY + sh;
        g.drawString(name, nameX, nameY);

        // center
        int xStart = (game.getScreenWidth() - (maxWidth * colCount)) / 2;
        int x = xStart;
        int y = (game.getScreenHeight() - (fm.getAscent() * rowCount)) / 2;
        int count = 0;
