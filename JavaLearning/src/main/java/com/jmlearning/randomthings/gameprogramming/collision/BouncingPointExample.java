...
public class BouncingPointExample extends SimpleFramework {
    ...
    private void createPoints() {
        ...
        for(int i = 0; i < points.length; ++i) {
            ...
            double distancePerSecond = random.nextFloat() + 0.5f;
            float vx = (float)(distancePerSecond * Math.cos(radians));
            float vy = (float)(distancePerSecond * Math.sin(radians));
    ...
    protected void updateObjects(float delta) {
        ...
        for(int i = 0; i < points.length; ++i) {
            ...
            Vector2f end = start.add(velocity.multiply(delta));
    ...
    private Vector2f getNewVelocity(
        ...
        for(int i = 0; i < polygon.length; ++i) {
            ...
            if(hitPolygon(start, end, S, P))
                return calcReflection(velocity, S, P);
    ...
    private Vector2f calcReflection(Vector2f V, Vector2f P0, Vector2f P1) {

        Vector2f Pv = P0.subtract(P1);
        Vector2f n = Pv.perp().norm();
        Vector2f Vn = n.multiply(V.dot(n));
        Vector2f Vp = V.subtract(Vn);

        return Vp.subtract(Vn);
    }

    private boolean hitPolygon(Vector2f start, Vector2f end,
                               Vector2f S, Vector2f P) {
        ...
        if(isZero(getPointLineDistance(end, S, P))) {
            ...
            return twoLineIntersect(start, end, S, P);
        }
        else {
    ...
    private float getPointLineDistance(Vector2f P, Vector2f S, Vector2f Q) {
        ...
        Vector2f v = Q.subtract(S);
        Vector2f n = v.perp().norm();

        return n.dot(P.subtract(Q));
    }

    private boolean twoLineIntersect(
    ...
    protected void render(Graphics g) {
        ...
        g.drawString("Down Arrow to decrease points", 20, 65);
        g.drawString("[Space] to reset points", 20, 80);
        Matrix3x3f view = getViewportTransform();

        for(int i = 0; i < points.length; ++i) {

            Vector2f copy = view.multiply(points[i]);
